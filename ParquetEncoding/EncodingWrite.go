...
import (
	...
	"github.com/lekki/parquet-go/Common"
...
func WriteDeltaINT32(nums []interface{}) []byte {
	...
	var totalNumValues = uint64(len(nums))

	num := nums[0].(int32)
	var firstValue = uint64((num >> 31) ^ (num << 1))
	...
	for i < len(nums) {
		...
		var minDeltaZigZag = uint64((minDelta >> 31) ^ (minDelta << 1))
...
func WriteDeltaINT64(nums []interface{}) []byte {
	...
	num := nums[0].(int64)
	var firstValue = uint64((num >> 63) ^ (num << 1))
	...
	for i < len(nums) {
		...
		var minDeltaZigZag = uint64((minDelta >> 63) ^ (minDelta << 1))
		res = append(res, WriteUnsignedVarInt(minDeltaZigZag)...)
		res = append(res, bitWidths...)
...
func WriteBitPackedDeprecated(vals []interface{}, bitWidth int64) []byte {
	...
	var valBitLeft = uint64(bitWidth)
	var val = valsInt[0] << uint64(64-bitWidth)
	for i < ln {
