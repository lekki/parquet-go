...
import (
	...
	"github.com/lekki/parquet-go/ParquetEncoding"
	"github.com/lekki/parquet-go/ParquetType"
	"github.com/lekki/parquet-go/SchemaHandler"
...
func PagesToChunk(pages []*Page) *Chunk {
	...
	var maxVal = pages[0].MaxVal
	var minVal = pages[0].MinVal
...
func PagesToDictChunk(pages []*Page) *Chunk {
	...
	var maxVal = pages[1].MaxVal
	var minVal = pages[1].MinVal
...
func ReadChunk(thriftReader *thrift.TBufferedTransport, schemaHandler *SchemaHandler.SchemaHandler, chunkHeader *parquet.ColumnChunk) (*Chunk, error) {
	...
	var numValues = chunkHeader.MetaData.GetNumValues()
